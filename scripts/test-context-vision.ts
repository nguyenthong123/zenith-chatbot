...
async function testContextVision() {
  ...
  const imageUrl = "https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&q=80&w=1000";

  const history: any[] = [
    {
      role: "user",
      content: [
        { type: "text", text: "[Kèm theo 1 ảnh] [HỆ THỐNG: Người dùng vừa gửi một ảnh]" },
        { type: "image", image: imageUrl }
      ]
    },
    {
      role: "assistant",
      content: "Tôi đã nhận được ảnh sản phẩm giày chạy bộ Nike Free màu đỏ. Bạn muốn:\n1. Lưu sản phẩm mới\n2. Cập nhật ảnh cho sản phẩm cũ\n3. Lưu minh chứng thanh toán"
    }
  ];
  ...
  try {
      ...
      *** XỬ LÝ ĐA PHƯƠNG TIỆN ***
      ...
      tools: {
        updateProductImage: tool({
          description: "Cập nhật ảnh cho sản phẩm hiện có. Hãy lấy URL ảnh từ lịch sử nếu người dùng vừa gửi.",
          inputSchema: z.object({
            productName: z.string().describe("Tên sản phẩm"),
            imageUrls: z.array(z.string()).describe("Danh sách link ảnh từ lịch sử")
          }),
    ...
    if (result.toolCalls && result.toolCalls.length > 0) {
      console.log(`Raw Tool Calls: ${JSON.stringify(result.toolCalls, null, 2)}`);
