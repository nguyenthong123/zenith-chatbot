...
export const getSaveProductTool = (
  userId: string,
  userEmail?: string,
) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      description: z.string().describe("A short introduction or description of the product (saved as 'note')."),
      imageUrls: z.array(z.string()).describe("An array of ALL URLs of the images to save. Collect every URL from all 'Attachment URLs (for tool use)' blocks in the conversation history."),
      category: z.string().optional().describe("Optional category for the product."),
    ...
    execute: async ({ name, description, imageUrls, category, sku }) => {
      try {
        ...
        console.log(`[saveProduct] Processing ${imageUrls.length} images:`, imageUrls);
        ...
        for (const [index, url] of imageUrls.entries()) {
          try {
            ...
            if (url.startsWith("data:")) {
              console.log(`[saveProduct] Detected data URL for image ${index}`);
              base64 = url;
            } else {
              console.log(`[saveProduct] Downloading image ${index} from: ${url}`);
              ...
              console.log(`[saveProduct] Downloaded image ${index}, size: ${buffer.length} bytes, type: ${contentType}`);
            }
            ...
            if (base64) {
              ...
              if (process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
                try {
                  console.log(`[saveProduct] Uploading image ${index} to Cloudinary...`);
                  ...
                  console.error(`[saveProduct] Cloudinary failed:`, err);
                }
              }
              ...
              if (!savedUrl && process.env.BLOB_READ_WRITE_TOKEN) {
                try {
                  ...
                  console.log(`[saveProduct] Uploading image ${index} to Vercel Blob...`);
                  const blob = await put(`products/${name}-${index}.png`, Buffer.from(base64.split(",")[1], "base64"), {
                    access: "public",
                    contentType: "image/png",
                  });
                  savedUrl = blob.url;
                } catch (err) {
                  console.error(`[saveProduct] Vercel Blob failed:`, err);
                }
              }
              ...
              if (!savedUrl && process.env.IMGBB_API_KEY) {
                try {
                  savedUrl = await uploadToImgBB(base64, process.env.IMGBB_API_KEY);
                } catch (err) {
                  console.error(`[saveProduct] ImgBB failed:`, err);
                }
              }
              ...
              if (savedUrl) {
                ...
                if (url.startsWith("data:")) {
                  ...
                  if (url.length < 20000000) {
                    console.log(`[saveProduct] Storage not configured or failed. Direct saving data URL ${index} (Length: ${url.length} chars).`);
                    finalImageUrls.push(url);
                  } else {
                    console.warn(`[saveProduct] Data URL ${index} too large for direct save (${url.length} chars) and no storage configured.`);
                  }
                } else {
            ...
            console.error(`[saveProduct] Error processing image ${index}:`, error);
        ...
        if (finalImageUrls.length === 0) {
          const configMsg = storageConfigured ? "" : " (Cấu hình Storage: Cloudinary/Vercel Blob chưa được thiết lập)";
          const diagnostics = imageUrls.map((url, i) => `[${i}]: len=${url.length}, start="${url.substring(0, 100)}"`).join(" | ");
          return { 
        ...
        console.log(`[saveProduct] Final image URLs (${finalImageUrls.length}):`, finalImageUrls);
        const combinedUrl = finalImageUrls.join(", ");
        const imageUrlsArray = finalImageUrls; // Still used for logging or future expansion

        console.log(`[saveProduct] Calling upsertProduct for "${name.trim()}" with ownerId: ${userId}`);
        ...
        console.log(`[saveProduct] Upsert successful:`, upsertResult);
        ...
        const hasFailedUploads = finalImageUrls.some((u, i) => u === imageUrls[i] && !u.startsWith('data:'));
