  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      imageUrls: z
        ...
        .optional()
        .describe(
          "An array of URLs of the images to save. Collect from 'Attachment URLs' blocks if available.",
        ),
      category: z
        .string()
        .optional()
    ...
    execute: async ({ name, description, imageUrls, category, sku }) => {
      try {
        ...
        // If imageUrls were provided but none were successfully saved, return error.
        if (imageUrls && imageUrls.length > 0 && finalImageUrls.length === 0) {
          return {
            error: "Failed to save images.",
            message: `Bạn đã cung cấp url ảnh nhưng hệ thống không thể tải và lưu được ảnh nào. Vui lòng thử lại với ảnh khác hoặc định dạng link công khai.`,
          };
        }
        ...
        const successMsg = hasFailedUploads
          ? `Sản phẩm "${name.trim()}" đã được lưu, NHƯNG ảnh đính kèm có chứa link không thể truy cập được. Hệ thống không thể tải và lưu ảnh này.`
          : `Sản phẩm "${name.trim()}" đã được lưu/cập nhật thành công.`;
